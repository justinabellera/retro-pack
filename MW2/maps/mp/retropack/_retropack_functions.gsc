...
SoftLandKillcamz()
{
	if(self.SoftLands == 1)
	{
		...
		setDvar( "bg_falldamagemaxheight", "300" );
        setDvar( "bg_falldamageminheight", "128" );
		self iPrintln("Killcam Only Softlands: ^1[Off]");
		self.softlandtext Destroy();
		self.SoftLandTextRetro = "^1Disabled";
		self.softlandtext = createFontString("DEFAULT", 1.0);
		self.softlandtext setPoint("LEFT", "CENTER", 80, -35);
	...
	else if(self.SoftLands == 0)
	{
		self.SoftLands = 1;
		self iPrintln("Killcam Only Softlands: ^2[On]");
		self.softlandtext Destroy();
		self.SoftLandTextRetro = "^2Killcam Only";
		self.softlandtext = createFontString("DEFAULT", 1.0);
		self.softlandtext setPoint("LEFT", "CENTER", 80, -35);
		self.softlandtext setText("Softland: " + self.SoftLandTextRetro + " ");
		self.softlandtext.archived = self.NotStealth;
...
SoftLandTest()
{
	...
	//{
		...
		self.scriptRide moveto(self.origin, 0);
		wait .00001;
...
AddBot(team, smart)
{
    ...
    for(i=0;i<int(1);i++)
    {
		...
		bot[i] setPlayerData( "prestige", randomInt(11));
		bot[i] freezeControls(true);
		bot[i] _clearPerks();
		bot[i] maps\mp\perks\_perks::givePerk("specialty_fastmantle");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_marathon");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_bulletaccuracy");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_holdbreath");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_improvedholdbreath");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_fastreload");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_fastsprintrecovery");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_automantle");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_bulletdamage");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_bulletpenetration");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_rof");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_lightweight");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_falldamage");
		bot[i] maps\mp\perks\_perks::givePerk("specialty_coldblooded");
        wait 0.1;
    }
}
...
doBotsShootL() //last stand
{
    ...
    for(;;)
    {
		...
		self maps\mp\perks\_perks::givePerk( "specialty_laststand" );
...
fakeLag(value)
{
	setDvar("sv_padPackets", value);
	self iprintLn ("^5Fake Lag: ^7" + value);
	level waittill("game_ended");
	wait 3;
