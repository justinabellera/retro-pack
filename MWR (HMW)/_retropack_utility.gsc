...
Notes:
- Added set_menu_animation() (when opening/closing menu)
*/
...
get_explosive_bullets_xp(player, weapon, type, victim) { //wip 
	value = getDvar("g_gametype") == "sd" ? 500 : 50;
	
	if ( getDvarInt("rp_firstblood") == 1 && level.numKills == 0) {
		if ( isDefined(player.pers["rp_firstblood"]) && player.pers["rp_firstblood"] && getDvar("g_gametype") == "sd" )
			value = value + 100;
		if ( isDefined(player.pers["rp_firstblood"]) && player.pers["rp_firstblood"] && getDvar("g_gametype") != "sd" )
			value = value + 50;
	}
	if ( maps\mp\_events::islongshot ( player, weapon, type, victim ) )
		value = value + 50;
	if ( type == "MOD_HEAD_SHOT" ) {
		if ( isDefined(player.pers["rp_headshots"]) && player.pers["rp_headshots"] && getDvar("g_gametype") == "sd" )
			value = value + 500;
		if ( isDefined(player.pers["rp_headshots"]) && player.pers["rp_headshots"] && getDvar("g_gametype") != "sd" )
			value = value + 50;
	}
...
get_localised_attachment(attachment) {
  if (attachment == "none" || !isDefined(attachment) || attachment == "") // using localised perks as a variable is buggy, have to hard-code.
    ...
    return "Akimbo";
  else if (attachment == "tacknifecolt44" || attachment == "tacknifedeagle" || attachment == "tacknifecolt45" || attachment == "tacknifem9" || attachment == "tacknifeusp" || attachment == "tacknifemp412")
...
set_timescale(timescale, spawn, bypass) {
  if (isDefined(spawn))
    wait 0.5;
  if (!isDefined(bypass))
...
set_slider(scrolling, index) {
  ...
  if(menu == "Velocity" || menu == "Bolt")
    storage = (menu + "_" + index + retropack_storage(1, menu));
  else
  ...
  if(menu == "Velocity" || menu == "Bolt") {
    if (!isDefined(self.pers["storage_increment" + menu + index + retropack_storage(1, menu)]) || isDefined(self.structure[index].execute) && !self.structure[index].execute)
      self.pers["storage_increment" + menu + index + retropack_storage(1, menu)] = self.structure[index].start;
  
	if (!isdefined(self.slider[storage]))
      self.slider[storage] = isdefined(self.structure[index].array) ? self.pers["storage_array" + menu + index] : self.pers["storage_increment" + menu + index + retropack_storage(1, menu)];
  } else {
  ...
  if (isdefined(self.structure[index].array)) {
	...
	if(menu == "Velocity" || menu == "Bolt")
	  self.pers["storage_increment" + menu + index + retropack_storage(1, menu)] = self.slider[storage];
	else
	...
	if(menu == "Velocity" || menu == "Bolt") {
      self.retropack["hud"]["slider"][0][index] setvalue(self.pers["storage_increment" + menu + index + retropack_storage(1, menu)]);
      self.retropack["hud"]["slider"][2][index].x = (self.retropack["hud"]["slider"][1][index].x + (abs((self.pers["storage_increment" + menu + index + retropack_storage(1, menu)] - self.structure[index].minimum)) / position) - 42);
	} else {
...
extract_attachment(weapon) {
  ...
  if (isEndStr(sep[0], "_tacknifecolt45"))
	...
	if (isEndStr(sep[0], "_tacknifemp412"))
    return "tacknifemp412";
...
create_button_struct(key, display) {
    s = spawnstruct();
    s.key = key;
    s.display = display;
    return s;
...
create_feature_struct(key, option_text, menu_name) {
    s = spawnstruct();
    s.key = key;
    s.option_text = option_text;
...
open_menu(menu) {
  if (!isdefined(menu))
    menu = isdefined(self get_menu()) && self get_menu() != "Home" ? self get_menu() : "Home";
  ...
  self.retropack["hud"]["background"][0] = self create_shader("white", "TOP", "TOP", 0, 74 * GetDvarFloat("safeArea_adjusted_vertical"), self.retropack["utility"].menu_width, self.retropack["utility"].menu_height, (0, 0, 0), 0.6, 1);
  self.retropack["hud"]["scroll"] = self create_shader("white", "TOP_RIGHT", "CENTER", (self.retropack["utility"].x_offset + (self.retropack["utility"].menu_width - 4)), self.retropack["utility"].y_offset + 14, 2, (self.retropack["utility"].menu_height - 2), (0.65, 0.65, 0.65), 0, 4);
  
	if(isDefined(self.pers["show_open"]) && self.pers["show_open"]) {
		self.retropack["retropack"]["controls"].alpha = 1;
		if (!is_player_gamepad_enabled())
			self.retropack["retropack"]["controls"] setText("[{+attack}] CONFIRM | [{+toggleads_throw}] BACK | [{+forward}] UP / [{+back}] DOWN | [{+moveleft}] / [{+moveright}] CHANGE VALUE");
		else
			self.retropack["retropack"]["controls"] setText("[{+usereload}] CONFIRM | [{+melee}] BACK | [{+actionslot 1}] UP / [{+actionslot 2}] DOWN | [{+actionslot 3}] / [{+actionslot 4}] CHANGE VALUE");
	}
	self set_menu(menu);
  self create_option(true);
  self set_state();
}

close_menu() {
	self set_menu_animation(false, 0, false);
  self clear_obituary();
  if (isdefined(self.pers["time_scale"])) {
  ...
  self set_state();
	if(isDefined(self.pers["show_open"]) && self.pers["show_open"]) {
		self.retropack["retropack"]["controls"].alpha = 1;
		if (is_player_gamepad_enabled())
			self.retropack["retropack"]["controls"] setText("PRESS [{+speed_throw}] + [{+actionslot 1}] FOR THE RETRO PACKAGE");
		else
			self.retropack["retropack"]["controls"] setText("PRESS [{+lookup}] FOR THE RETRO PACKAGE");
	}
  self notify("end_menu");
}

set_menu_animation(value, time_offset, bypass) {
	hud_elements = [];
  if (isdefined(self.retropack["hud"])) {
		hud_elements[hud_elements.size] = self.retropack["hud"]["title"][0];
		hud_elements[hud_elements.size] = self.retropack["hud"]["title"][1];
		hud_elements[hud_elements.size] = self.retropack["hud"]["background"][0];
    for (i = 0; i < self.retropack["utility"].element_list.size; i++) {
      category = self.retropack["utility"].element_list[i];
      if (isdefined(self.retropack["hud"][category])) {
        if (category == "scroll") {
          hud_elements[hud_elements.size] = self.retropack["hud"]["scroll"];
        } else if (category == "text" || category == "slider" || category == "category") {
          for (j = 0; j < self.structure.size; j++) {
            if (isdefined(self.retropack["hud"][category][j])) {
              for (k = 0; k < self.structure.size; k++) {
                if (isdefined(self.retropack["hud"][category][j][k])) {
                  hud_elements[hud_elements.size] = self.retropack["hud"][category][j][k];
                }
              }
            }
          }
          ...
          for (j = 0; j < self.structure.size; j++) {
            if (isdefined(self.retropack["hud"][category][j])) {
              hud_elements[hud_elements.size] = self.retropack["hud"][category][j];
            }
          }
        }
      }
    }
    for (i = 0; i < hud_elements.size; i++) {
      if (isdefined(hud_elements[i])) {
				if(!bypass)
					hud_elements[i] moveOverTime(self.retropack["utility"].animation_duration);
				if(isDefined(value) && value) {
					hud_elements[i].y = hud_elements[i].y - self.retropack["utility"].offscreen_y_offset;
				} else {
					hud_elements[i].y = hud_elements[i].y + self.retropack["utility"].offscreen_y_offset;
				}
      }
		...
		if(!bypass) {
			if(!value)
				self.retropack["hud"]["scroll"].alpha = 0;
			
			wait(self.retropack["utility"].animation_duration + time_offset);
			
			if(isDefined(value) && value)
				self.retropack["hud"]["scroll"].alpha = 1;
		}
  }
}
  ...
  if (isdefined(self.structure) && self.structure.size) {
    ...
    for (i = 0; i < limit; i++) {
      ...
      if (isdefined(self.structure[index].function) && self.structure[index].function == ::new_menu && !isdefined(self.structure[index].labelled) && !isdefined(self.structure[index].shaderlabelled))
        self.retropack["hud"]["submenu"][index] = self create_shader("ui_arrow_right", "TOP_RIGHT", "CENTER", (self.retropack["utility"].x_offset + (self.retropack["utility"].menu_width - 13)), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 17)), 7, 7, color[0], 1, 10);
      
	  if (self get_menu() == "Binds" && isdefined(self.structure[index].labelled)) {
        self.retropack["hud"]["submenu"][index] = self create_shader("ui_arrow_right", "TOP_RIGHT", "CENTER", (self.retropack["utility"].x_offset + 10), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 17)), 7, 7, color[0], 1, 10);
	  }

      if (isdefined(self.structure[index].toggle))
        self.retropack["hud"]["toggle"][index] = self create_shader("white", "TOP_LEFT", "CENTER", (self.retropack["utility"].x_offset + (self.retropack["utility"].menu_width - 18)), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 17)), 8, 8, color[1], 1, 10);
      ...
      if (isdefined(self.structure[index].shaderlabelled)){
		  ...
		  self.retropack["hud"]["slider"][4][index] = self create_shader(self.structure[index].shaderlabelled_2, "TOP_RIGHT", "CENTER", (self.retropack["utility"].x_offset + (self.retropack["utility"].menu_width - 12) - self.structure[index].sizex), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 15)), 12, 12, undefined, 1, 10);
	  }
      ...
      if (isdefined(self.structure[index].shaderoption) && !self.structure[index].shaderarray) {
	    if (cursor) {
		  ...
		  self.retropack["hud"]["text"][2][index] = self create_shader(self.structure[index].shader, "TOP_LEFT", "CENTER", (self.retropack["utility"].x_offset + 14), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 16)), self.structure[index].sizex, self.structure[index].sizey, (0, 0.835294, 1), 1, 10);
	    } else {
		  self.retropack["hud"]["text"][2][index] destroy_element();
		  self.retropack["hud"]["text"][2][index] = self create_shader(self.structure[index].shader, "TOP_LEFT", "CENTER", (self.retropack["utility"].x_offset + 14), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 16)), self.structure[index].sizex, self.structure[index].sizey, undefined, 1, 10);
	    }
	  }
        ...
        if (isdefined(self.structure[index].array)) {
          if (isdefined(self.structure[index].shaderarray)) {
            ...
            if (cursor) {
              self.retropack["hud"]["slider"][3][index] destroy_element();
              if (self get_menu() == "Camo") {
                self.retropack["hud"]["slider"][3][index] = self create_shader(sep[1], "TOP_RIGHT", "CENTER", (self.retropack["utility"].x_offset + (self.retropack["utility"].menu_width - 12)), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 15)), 12, 12, undefined, 1, 10);
          ...
          if (cursor)
            self.retropack["hud"]["slider"][0][index] = self create_text(self.slider[self get_menu() + "_" + index], self.retropack["utility"].font, (self.retropack["utility"].font_scale - 0.1), "CENTER", "CENTER", (self.retropack["utility"].x_offset + (self.retropack["utility"].menu_width - 35)), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 21)), (1, 1, 1), 1, 10);

          self.retropack["hud"]["slider"][1][index] = self create_shader("white", "TOP_RIGHT", "CENTER", (self.retropack["utility"].x_offset + (self.retropack["utility"].menu_width - 10)), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 17)), 50, 8, (0, 0, 0), 1, 8);
          self.retropack["hud"]["slider"][2][index] = self create_shader("white", "TOP_RIGHT", "CENTER", (self.retropack["utility"].x_offset + (self.retropack["utility"].menu_width - 52)), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 17)), 8, 8, cursor ? (0, 0.835294, 1) : (0.5, 0.5, 0.5), 1, 9);
        }
      ...
      if (return_toggle(self.structure[index].category)) {
        self.retropack["hud"]["category"][0][index] = self create_text(self.structure[index].text, self.retropack["utility"].font, self.retropack["utility"].font_scale, "CENTER", "CENTER", (self.retropack["utility"].x_offset + ((self.retropack["utility"].menu_width / 2) - 5)), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 21)), (0, 0.835294, 1), 1, 10);
        self.retropack["hud"]["category"][1][index] = self create_shader("white", "TOP_LEFT", "CENTER", (self.retropack["utility"].x_offset + 4), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 21)), 30, 1, (0, 0.835294, 1), 1, 10);
        self.retropack["hud"]["category"][2][index] = self create_shader("white", "TOP_RIGHT", "CENTER", (self.retropack["utility"].x_offset + (self.retropack["utility"].menu_width - 10)), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 21)), 30, 1, (0, 0.835294, 1), 1, 10);
      } else if (return_toggle(self.structure[index].divider)) {
        self.retropack["hud"]["divider"][0][index] = self create_shader("white", "TOP_LEFT", "CENTER", (self.retropack["utility"].x_offset + 4), (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 21)), 135, 1, (0, 0.835294, 1), 1, 10);
      } else {
        self.retropack["hud"]["text"][0][index] = self create_text(return_toggle(self.structure[index].slider) ? self.structure[index].text + ":" : self.structure[index].text, self.retropack["utility"].font, self.retropack["utility"].font_scale, "TOP_LEFT", "CENTER", self.retropack["utility"].x_offset + 4, (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + ((i * self.retropack["utility"].option_spacing) + 16)), (1, 1, 1), 1, 10);
	...
	if(isDefined(animation) && animation) {
		self set_menu_animation(false, 0, true);
		self set_menu_animation(true, 0, false);
	}
  self update_resize();
}
...
update_resize() {
  ...
  if (!isdefined(self.retropack["hud"]["scroll"]))
    self.retropack["hud"]["scroll"] = self create_shader("white", "TOP_RIGHT", "CENTER", (self.retropack["utility"].x_offset + (self.retropack["utility"].menu_width - 4)), (self.retropack["utility"].y_offset + 14), 2, 0, (0.65, 0.65, 0.65), 1, 4);

  self.retropack["hud"]["background"][0] scaleOverTime(.1, self.retropack["hud"]["background"][0].width, return_toggle(self.structure.size > self.retropack["utility"].option_limit) ? self.retropack["utility"].menu_height : (23 + self.retropack["utility"].menu_title_gap + (self.structure.size * self.retropack["utility"].option_spacing)));
  ...
  self.retropack["hud"]["scroll"].y = (self.retropack["utility"].menu_title_gap + self.retropack["utility"].y_offset + 15);
  if (self.structure.size > self.retropack["utility"].option_limit)
...
add_bind(text, function, bind, argument_1, argument_2, argument_6, argument_7, argument_8) {
  option = spawnstruct();
  option.text = text;
  option.bind = true;
  ...
  option.argument_6 = argument_6;
  option.argument_7 = argument_7;
  option.argument_8 = argument_8;

  self.structure[self.structure.size] = option;
}

add_shaderlabel(text, shader, shader_2, x, y, function, argument_1, argument_2, argument_3, argument_4, argument_5, argument_6, argument_7, argument_8) {
  option = spawnstruct();
  option.text = text;
  ...
  option.argument_6 = argument_3;
  option.argument_7 = argument_4;
  option.argument_8 = argument_5;
...
add_label(text, label, function, argument_1, argument_2, argument_3, argument_4, argument_5, argument_6, argument_7, argument_8) {
  option = spawnstruct();
  option.text = text;
  option.function = function;
  option.argument_1 = argument_1;
...
add_shaderarray(text, execute, function, array, argument_1, argument_2, argument_3, argument_4, argument_5, argument_6, argument_7, argument_8) {
  option = spawnstruct();
  option.text = text;
...
add_option(text, function, argument_1, argument_2, argument_3, argument_4, argument_5, argument_6, argument_7, argument_8) {
  option = spawnstruct();
  option.text = text;
  option.function = function;
  option.argument_1 = argument_1;
...
add_toggle(text, function, toggle, array, argument_1, argument_2, argument_3, argument_4, argument_5, argument_6, argument_7, argument_8) {
  option = spawnstruct();
  option.text = text;
  option.function = function;
...
add_togglemenu(text, function, toggle, argument_1, argument_2, argument_3, argument_4, argument_5, argument_6, argument_7, argument_8) {
  option = spawnstruct();
  option.text = text;
  option.function = function;
  option.toggle = return_toggle(toggle);
...
add_integer(text, label, hover, execute, function, array, argument_1, argument_2, argument_3, argument_4, argument_5, argument_6, argument_7, argument_8) {
  option = spawnstruct();
  option.text = text;
  option.integer = true;
  option.hover = hover;
...
add_string(text, execute, function, array, argument_1, argument_2, argument_3, argument_4, argument_5, argument_6, argument_7, argument_8) {
  option = spawnstruct();
  option.text = text;
  option.execute = execute;
  option.function = function;
...
add_increment(text, execute, function, start, minimum, maximum, increment, argument_1, argument_2, argument_3, argument_4, argument_5, argument_6, argument_7, argument_8) {
  option = spawnstruct();
  option.text = text;
  option.execute = execute;
  option.function = function;
