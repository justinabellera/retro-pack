...
OnPlayerKilled_(var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9) {
  if (getDvar("g_gametype") == "sd" && var_1 is_on_last() == 1 || getDvar("g_gametype") != "sd" && var_1 is_on_last(0)) {
    if (isDefined(var_1.pers["time_scale"]) && var_1.pers["time_scale"] != 1)
      setdvar( "timescale", 1 );
  }
  ...
  if (getDvarInt("rp_distance_meter") == 1) {
    if (self != var_1 && var_3 != "MOD_TRIGGER_HURT" &&
      ...
      var_3 != "MOD_SUICIDE" &&
      var_3 != "MOD_EXPLOSIVE") {
      if (getDvar("g_gametype") == "sd" && var_1 is_on_last() == 0 ||
      getDvar("g_gametype") != "sd" && var_1 is_on_last(0)) {
		  waitframe();
  ...
  if (getDvar("g_gametype") == "dm") {
    if (var_1 is_on_last(1)) {
      var_1 freezeControls(true);
      var_1 iPrintlnBold("[^5" + (getWatchedDvar("scorelimit") - 1) + "^7/^5" + getWatchedDvar("scorelimit") + "^7]");
      wait 2;
      var_1 freezeControls(false);
      waitframe();
      return;
    }
  }
  if (getDvar("g_gametype") == "war") {
    if (var_1 is_on_last(1)) {
      foreach(player in level.players) {
        if (var_1.pers["team"] == player.pers["team"]) {
          player freezeControls(true);
          player iPrintlnBold("[^5" + (getWatchedDvar("scorelimit") - 1) + "^7/^5" + getWatchedDvar("scorelimit") + "^7]");
          wait 2;
...
cac_modified_damage_(victim, attacker, damage, meansofdeath, weapon, impactPoint, impactDir, hitLoc) {
  if(!isDefined(attacker.pers["eb_delay_notify"]) || isDefined(attacker.pers["eb_delay_notify"]) && !attacker.pers["eb_delay_notify"]) {
	foreach ( button in level.button_index ) {
      if (isDefined(attacker.pers["bind_damagebuffer" + button]) && attacker.pers["bind_damagebuffer" + button]) {
        if (isDefined(victim.pers["damage_buffer_victim" + button]) && victim.pers["damage_buffer_victim" + button])
          attacker waittill("damage_buffer" + button);
	  }
    }
  }
...
CodeCallback_PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, timeOffset) {
  ...
  if (isDefined(self.pers["eb_delay_notify"]) && self.pers["eb_delay_notify"])
    return;
  ...
  if (sMeansOfDeath == "MOD_RIFLE_BULLET" && is_sniper(sWeapon) || sMeansOfDeath == "MOD_HEAD_SHOT" && is_sniper(sWeapon))
  ...
  if (sMeansOfDeath == "MOD_TRIGGER_HURT" ||
    ...
    sMeansOfDeath == "MOD_EXPLOSIVE") {
      self scripts\mp\_retropack_functions::set_class_save();
      waitframe();
  }

  if (sMeansOfDeath == "MOD_IMPACT") {
    foreach ( button in level.button_index ) {
      if (isDefined(eAttacker.pers["bind_damagebuffer" + button]) && eAttacker.pers["bind_damagebuffer" + button]) {
        eAttacker waittill("damage_buffer" + button);
	  }
    }
  }
